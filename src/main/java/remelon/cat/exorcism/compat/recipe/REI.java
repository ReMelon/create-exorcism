...
import remelon.cat.exorcism.compat.recipe.category.ExorcisingCategoryREI;
...
public class REI implements REIClientPlugin {
	private static final Identifier REIID = Exorcism.GenID("rei_plugin");
	...
	private class CategoryBuilder<T extends Recipe<?>> {
		...
		public CreateRecipeCategory<T> build(String name, CreateRecipeCategory.Factory<T> factory) {
			...
			if (width <= 0 || height <= 0) {
				Exorcism.LOGGER.warn("Create REI category [{}] has weird dimensions: {}x{}", name, width, height);
			}

			CreateRecipeCategory.Info<T> info = new CreateRecipeCategory.Info<>(
					CategoryIdentifier.of(Exorcism.GenID(name)),
					Lang.translateDirect("recipe." + name), background, icon, recipesSupplier, catalysts, width, height, displayFactory == null ? (recipe) -> new CreateDisplay<>(recipe, CategoryIdentifier.of(Exorcism.GenID(name))) : displayFactory);
